...
The `join_spike` contract in this project is a spike stub: it exists to prove whether an atomic
transaction group carrying an extra buyer-signed application call, alongside a standard payment, is
internally well-formed and accepted by the AVM — not to be the production escrow application.
Deliberately absent: roster management, quorum/release/refund logic, and `record_join`.
...
- **Setup Project**: Execute `algokit project bootstrap all` to install dependencies and setup a Python virtual environment in `.venv`.
  **Deactivate any other virtualenv first** — Poetry installs into an active `VIRTUAL_ENV` instead of creating `contracts/.venv`, which merges this surface with the repository-root one. See *Before any of those: `contracts/` needs its own virtualenv* in `AGENTS.md` for why a missing `contracts/.venv` makes `poetry run` succeed while `algokit project run` fails.
...
This project has one contract, `JoinSpike`, under the `join_spike` folder in the `smart_contracts` directory. To add a new contract:
