...
class BuildError(Exception):
    """A build, client-generation or deployment step could not complete."""
...
def import_contract(folder: Path) -> Path:
    ...
    if contract_path.exists():
        ...
        raise BuildError(f"Contract not found in {folder}")
...
def build(output_dir: Path, contract_path: Path) -> Path:
    ...
    if build_result.returncode:
        raise BuildError(f"Could not build contract:\n{build_result.stdout}")
        ...
        case "deploy":
            for contract in filtered_contracts:
                ...
                if app_spec_file_name is None:
                    raise BuildError("Could not deploy app, .arc56.json file not found")
                if contract.deploy:
